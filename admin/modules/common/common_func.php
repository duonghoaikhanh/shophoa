...
class commonFunc extends sMain 
{
  ...
  //=================load_setting===============
	...
	function _form_input($data)
	{
		...
		foreach($this->arr_element as $k => $v) {
			...
			switch ($form_type) {
				case "picture":
					...
					break;
				case "file":
					$data_out[$k] = $ttH->admin->get_form_file ($k, (isset($data[$k]) ? $data[$k] : ''), $this->folder_upload, $this->dir);
					break;
				case "select":
					...
					$attr  = (isset($v['attr']) ? $v['attr'] : " class=\"form-control\"");
					$tmp = isset($v['arr_more']) ? $v['arr_more'] : array();
					
					if(isset($tmp['parent_code'])) {
						$tmp['parent_code'] = (isset($data[$tmp['parent_code']])) ? $data[$tmp['parent_code']] : '';
					}
					...
					if(isset($v['data']) && is_array($v['data'])) {
						...
						$data_out[$k] = $ttH->admin->list_number (
							(isset($tmp["select_muti"]) ? $k.'[]' : $k), 
							(isset($v['min']) ? $v['min'] : 1), 
							(isset($v['max']) ? $v['max'] : 100), 
							(isset($data[$k]) ? $data[$k] : ''), 
						...
						$func_call = (isset($v['function_call']) && $v['function_call']) ? $v['function_call'] : 'list_'.$k;	
						eval('$data_out["'.$k.'"] = $this->'.$func_call.' (
						"'.(isset($tmp["select_muti"]) ? $k.'[]' : $k).'", 
						"'.$data[$k].'", 
						$attr, 
						$tmp);');
				...
				case "editor":
					$form_col = 12;
					$data_out[$k] = $ttH->editor->load_editor (
						$k, 
						$k, 
						(isset($data[$k]) ? $data[$k] : ''), 
						"", 
				...
				case "friendly_link":
					...
					$data_out[$k] = $ttH->html->temp_box('input_text', array(
						'key' => $k,
				...
				case "meta_title":
					...
					break;
				case "meta_key":
					$data_out[$k] = $ttH->html->temp_box('textarea', array(
						'key' => $k,
						'rows' => 1,
						'content' => (isset($data[$k]) ? $data[$k] : ''),
						'ext' => '<p>'.$ttH->lang['global'][$form_type.'_note'].'</p>'
				...
				case "map_google":
					$form_col = 12;
					$data_out[$k] = $ttH->html->temp_box('map_google', array(
						'pre' => $k.'_',
						'map_latitude' => (isset($data[$v['map_latitude']]) ? $data[$v['map_latitude']] : ''),
						'map_longitude' => (isset($data[$v['map_longitude']]) ? $data[$v['map_longitude']] : '')
					));
					break;
				/*case "input_option":
					$data_out[$k] = $this->list_input_option(
						$k,
						$data[$k],
						$attr,
						$tmp);
                    $data_out[$k] = $ttH->html->list_input_option (
                        (isset($v['muti'])) ? $k.'[]' : $k,
                        ((isset($v['data']) && is_array($v['data'])) ? $v['data'] : array(1 => $title)),
                        (isset($data[$k]) ? $data[$k] : ''),
                        $tmp
				...
				default:
					...
					$data_out[$k] = $ttH->html->temp_box($tmp_temp, array(
						'key' => $k,
						'content' => (isset($data[$k]) ? $data[$k] : ''),
			...
			$col_title = (isset($ttH->lang[$this->modules][$k]) ? $ttH->lang[$this->modules][$k] : (isset($ttH->lang['global'][$k]) ? $ttH->lang['global'][$k] : $k));
			...
			if(isset($data_out[$k])) {
				$ttH->temp_act->assign('row', array(
					...
					'key' => $k,
					'title' => $col_title,
					'content' => (isset($data_out[$k]) ? $data_out[$k] : '')
				));
				
				$use_title = (isset($v['use_title'])) ? $v['use_title'] : $use_title;
				if($use_title == true) {
		...
		$ttH->temp_act->assign('data', $data_out);
		...
		if($required && is_array($v['required'])) {
			foreach($v['required'] as $k1 => $v1) {
				if($k1 === 'exit' || $v1 === 'exit') {
					...
					if(is_array($v1)) {
						foreach($v1 as $kt1 => $vt1) {
							$where .= " and ".$vt1."='".trim($ttH->post[$vt1])."'";
						}
					}
	...
	function _inputdb($k, $v) {
		...
		switch ($form_type) {
			...
			case "meta_desc":
				...
				break;
			case "input_option":
				$output = (isset($ttH->post[$k])) ? serialize($ttH->post[$k]) : '';
				break;
			default:
				if(!$form_type) {
					break;
		...
		$output = (!is_array($output)) ? trim($output) : $output;
	...
	function update_menu_link($name_action="", $info = array()) {
		...
		$tmp[1] = ($tmp[1] != $tmp[0]) ? $tmp[1] : 'item';
		$name_action = implode('-',$tmp);
		$add_act = $tmp[1];
	...
	function list_input_option ($input_name="arr_option", $cur=array(), $ext="",$arr_more=array()) {
		global $ttH;
		...
		$data = $ttH->load_data->data_table (
			$this->modules."_option", 
			"option_id", 
			"option_id, title", 
			"is_show=1 and lang='".$ttH->conf["lang_cur"]."' order by show_order desc, date_create asc");
		
		if(isset($data) && count($data) > 0){
			$ttH->temp_box->reset("input_option");
			$ttH->temp_box->reset("row");
			
			foreach($data as $row) {
				$row['input_name'] = $input_name.'['.$row['option_id'].']';
				$row['content'] = (isset($cur[$row['option_id']])) ? $cur[$row['option_id']] : '';
				$ttH->temp_box->assign('row', $row);
				$ttH->temp_box->parse("input_option.row");
			}
			
			$ttH->temp_box->parse("input_option");
			return $ttH->temp_box->text("input_option"); die();
		}
	...
	function list_location_country ($select_name="country_code", $cur="", $ext="",$arr_more=array()) {
		global $ttH;
		
		$parent_code = (isset($arr_more['parent_code'])) ? $arr_more['parent_code'] : '';
	...
	function list_location_province ($select_name="province_code", $cur="", $ext="",$arr_more=array()) {
		global $ttH;
	...
	function list_location_district ($select_name="district_code", $cur="", $ext="",$arr_more=array()) {
		global $ttH;
	...
	function list_location_ward ($select_name="ward_code", $cur="", $ext="",$arr_more=array()) {
		global $ttH;
